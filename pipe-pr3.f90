...
integer, parameter :: nx=5, ny=5, nu = (nx)*(ny-1), nv = (nx-1)*(ny-2), np = (nx-2)*(ny-2)
...
a(1:nx-2,1:ny-3,2) = rho*arx(1,1)*du(2:nx-1,2:ny-2)
a(1:nx-2,2:ny-2,3) = rho*arx(1,1)*du(2:nx-1,2:ny-2)
a(1:nx-3,1:ny-2,4) = rho*ary(1,1)*dv(2:nx-2,1:ny-2)
a(2:nx-2,1:ny-2,5) = rho*ary(1,1)*dv(2:nx-2,1:ny-2)
a(1:nx-2,1:ny-2,1) = a(1:nx-2,1:ny-2,2) + a(1:nx-2,1:ny-2,3) + &
 a(1:nx-2,1:ny-2,4) + a(1:nx-2,1:ny-2,5)

!a(1:nx,1,1) = a(1:nx,1,1) + po*rho*arx(1,1)*du(1:nx,1)

bd(1:nx-2,1:ny-2) = rho*arx(1,1)*(ust(2:nx-1,1:ny-2)-ust(2:nx-1,2:ny-1)) + &
rho*ary(1,1)*(vst(2:nx-1,2:ny-1)-vst(3:nx,2:ny-1))
...
pcr = 0.0
do i=1,nx-2
	pcr(i+1,2:ny-1) = bp(((i-1)*(ny-2)+1):(i)*(ny-2))
enddo
pcr(1,:)=pcr(2,:); pcr(nx,:)=pcr(nx-1,:)
!pcr(:,ny-1)=pcr(:,ny-2)
