...
integer, parameter :: nx=10, ny=10, nu = (nx)*(ny-1), nv = (nx-1)*(ny), np = (nx)*(ny-1)
...
au(:,:,:) = 0.0
au(1:nx,2:ny-2,2) = Dix(1:nx,3:ny-1) + max(-Fxu(1:nx,3:ny-1),0.0)
...
au(1,1:ny-1,5) + Fxu(1,2:ny) - Fxu(1,1:ny-1)  + Fyu(1,1:ny-1))/uru! + Fyu(1,1:ny-1)
au(nx,1:ny-1,1) = (au(nx,1:ny-1,2) + au(nx,1:ny-1,3) + au(nx,1:ny-1,4) + &
au(nx,1:ny-1,5) + Fxu(nx,2:ny) - Fxu(nx,1:ny-1)  - Fyu(nx-1,1:ny-1))/uru! - Fyu(nx-1,1:ny-1)
au(1:nx,1,1) = (0.5*Fxu(1:nx,2) + Fxu(1:nx,1) &
)/uru
...
Fyv(2:nx,1) - Fyv(1:nx-1,1) + Fxv(1:nx-1,1))/uru !+ Fxv(1:nx-1,1)
av(1:nx-1,ny,1) = (av(1:nx-1,ny,2) + av(1:nx-1,ny,3) + av(1:nx-1,ny,4) + av(1:nx-1,ny,5) + &
Fyv(2:nx,ny) - Fyv(1:nx-1,ny) - Fxv(1:nx-1,ny-1))/uru !- Fxv(1:nx-1,ny-1)
av(1:nx-1,1,1) = av(1:nx-1,1,1) + (rho*arx(1,1)*ust(1:nx-1,1))/uru
...
enddo
!pcr(1,1)=0.0; pcr(nx,1)=0.0

p(1:nx,1:ny-1) = (1-urp)*pst(1:nx,1:ny-1) + urp*pcr(1:nx,1:ny-1)
p(:,ny)=0.0
!p(:,1)=1.0
!p(1:nx,ny-1) = -10.0 ! 2*p(1:nx,ny-1) - p(1:nx,ny-2)
