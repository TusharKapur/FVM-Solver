...
double precision, parameter :: rho=997.0, tau = 0.001794
integer, parameter :: nx=30, ny=30, nu = (nx)*(ny-1), nv = (nx-1)*(ny), np = (nx)*(ny)
...
bd(nx,ny), bp(nx,ny), bu(nx,ny-1), bv(nx-1,ny), start, finish
integer :: i, j, k, nt, time, nrhs, info, n, iteru, iterv, iterp
...
do i = 1,nx
	do j = 1,ny
		pcr(i,j) = bp(i,j)
		if(j==1) then
			pcr(i,j) = pcr(i,j) + a(i,j,2)*pcr(i,j+1)
		else if(j==ny) then
			pcr(i,j) = pcr(i,j) + a(i,j,3)*pcr(i,j-1)
		else if(j>1 .and. j<ny) then
			pcr(i,j) = pcr(i,j) + a(i,j,2)*pcr(i,j+1) + a(i,j,3)*pcr(i,j-1)
		end if

		if(i==1) then
			pcr(i,j) = pcr(i,j) + a(i,j,4)*pcr(i+1,j)
		else if(i==ny) then
			pcr(i,j) = pcr(i,j) + a(i,j,5)*pcr(i-1,j)
		else if(i>1 .and. i<ny) then
			pcr(i,j) = pcr(i,j) + a(i,j,4)*pcr(i+1,j) + a(i,j,5)*pcr(i-1,j)
		end if

		pcr(i,j) = pcr(i,j)/a(i,j,1)
	enddo
enddo
